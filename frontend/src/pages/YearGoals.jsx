...
const YearGoals = ({ goalInfo, handleComplete, handleDelete, newGoal, setNewGoal, handleNewGoal }) => {

    let completionMsg;
    if (goalInfo.isEmpty.yearly) {
        completionMsg = <p>You haven't added any goals yet!</p>
    } else if (!goalInfo.incompleteYearlyGoals) {
        completionMsg = <p>You've completed all your yearly goals!</p>
    }

    const goalArray = Array.isArray(goalInfo.goals) ? goalInfo.goals : [];

    const filteredGoals = goalArray.filter(goal => 
        goal.type === 'yearly' && goal.status === false
    );
            ...
            <div>
                <table>
                    ...
                    <div class='new-task'>
                        ...
                        <button onClick={e => handleNewGoal('yearly')}>+</button>
                    </div>
            </div>
        </section>
    )
